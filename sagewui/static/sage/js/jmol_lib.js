(function(app) {
    app.JmolManager = {
        ...
        ready_callback: function (name, applet) {
            ...
            this.enforce_limit();
            jQuery('#'+name).parent().append(
                '<div id="'+name+'_hint">Right-click to get options menu.</div>');
        },
        ...
        activator: function () {
            if (document.getElementById("loadJmol")) {
                ...
                this.add_applet(size, img, script, server_url, cell_num);
    ...
    if (typeof(Jmol) !== 'undefined') {
        app.JmolManager._prepare_jmol();
        app.JmolManager._watcher = setInterval(
            app.JmolManager.activator.bind(app.JmolManager), 500);
    };
